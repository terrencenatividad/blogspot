						...
						<div class="col-md-6">
							<?php
								echo $ui->formField('dropdown')
									...
									->setName('stat')
									->setId('stat')
									->setList(array('Returned' => 'Returned', 'Scrapped' => 'Scrapped'))
									->setValue($stat)
									->setValidation('required')
									// ->addHidden(($ajax_task != 'ajax_create'))
									->draw($show_input);
			...
			<table id="tableList" class="table table-hover table-sidepad only-checkbox full-form">
				<thead>
					<tr class="info">
						...
						<?php if ($show_input): ?>
						<th class="col-xs-1 text-right">SalesR. Qty</th>
...
function displayDetails(details) {
	...
	if (details.length > 0) {
		...
		$('#tableList tbody').append(`
			<tr>
				<td colspan="10" class="text-center"><b>Select Sales Return No.</b></td>
			</tr>
		`);
	}
	<?php if ($show_input): ?>
