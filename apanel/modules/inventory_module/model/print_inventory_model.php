...
class print_inventory_model extends fpdf {
	...
	public function header() {
		...
		$source			= (isset($this->customer_details->source))		? $this->customer_details->source		: 	'';	
		$destination	= (isset($this->customer_details->destination))	? $this->customer_details->destination	: 	'';
		$remarks		= (isset($this->customer_details->remarks))		? $this->customer_details->remarks		: 	'';

		$this->Rect($this->margin_side, $detail_start, $detail_width, 24);
		$this->SetY($detail_start + 1);
		$this->SetFont('Arial', 'B', 8);
		$this->Cell(30, 4, 'REQUESTED FROM', 0, 0, 'L');
		$this->Cell(30, 4, ':', 0, 0, 'L');
		$this->SetFont('Arial', '', 8);
		$this->SetX(43);
		$this->MultiCell(100, 4, $source, 0, 'L');
		$this->SetFont('Arial', 'B', 8);
		$this->Cell(30, 4, 'TRANSFERRED TO', 0, 0, 'L');
		$this->Cell(30, 4, ':', 0, 0, 'L');
		$this->SetFont('Arial', '', 8);
		$this->SetX(43);
		$this->MultiCell(100, 4, $destination, 0, 'L');
		$this->SetFont('Arial', 'B', 8);
		$this->Cell(30, 4, 'REMARKS', 0, 0, 'L');
		$this->Cell(30, 4, ':', 0, 0, 'L');
		$this->SetFont('Arial', '', 8);
		$this->SetX(43);
		$this->MultiCell(100, 4, $remarks, 0, 'L');
		// $this->SetFont('Arial', 'B', 8);
		...
		if ($this->has_received) {
			if ($this->has_footer_details) {
				...
				if ($this->footer_details) {
					...
					$this->SetX($this->margin_side);
					foreach ($this->footer_details as $footer_index => $footer_value) {
						$this->SetFont('Arial', 'B', 8);
						if($this->isAssoc($this->footer_details)){
							$this->Cell(30, 5, $footer_index, 0, 0, 'L');
							$this->Cell(30, 5, $footer_value, 0, 0, 'L');
						}else{
							$this->Cell(30, 5, $footer_value, 0, 0, 'L');
						}
						$this->Ln();
