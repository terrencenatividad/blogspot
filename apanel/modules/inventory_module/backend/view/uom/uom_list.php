...
<div class="delete-modal">
	...
	<script>
		...
		$('#tableList').on('click', '.activate', function() { 
			...
			$.post('<?=MODULE_URL?>ajax/ajax_edit_activate', '&uomcode='+activate_code ,function(data) {
				getList();
			});
		});
		var deactivate_code = '';
		$('#tableList').on('click', '.deactivate', function() { 
			...
			$('#deactivate_modal').on('click', '#deactyes', function() {
				...
				$.post('<?=MODULE_URL?>ajax/ajax_edit_deactivate', '&uomcode='+deactivate_code ,function(data) {
					getList();
				});
			});
		});
