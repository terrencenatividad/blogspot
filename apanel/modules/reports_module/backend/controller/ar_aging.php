...
class controller extends wc_controller {
	...
	public function view() {
		$this->view->title = 'Accounts Receivable Aging';
		$data['ui'] = $this->ui;
		$data['show_input'] = true;
		$data['datefilter'] = date("M d, Y");
		$data['customer_list'] = $this->ar_aging->getCustomerList();
		$this->view->load('ar_aging', $data);
	}
	...
	private function ajax_list()
	{
		...
		$pagination = $this->ar_aging->getArAging($datefilter, $partnerfilter);
		...
		if(count($pagination->result)>0)
		{
			...
			{	
				...
				$diff 			= $this->ar_aging->dateDiff($datefilter,$duedate);
				$dateDelayed   	= $diff;
				...
				// if($balance > 0){
					...
					$tablerow	.= '<td class="center" style="vertical-align:middle;">'.date("M d, Y",strtotime($duedate)).'</td>';
					$tablerow	.= ($dateDelayed < 0) ? '<td class="text-right" style="vertical-align:middle;">'.number_format($balance,2).'</td>' : '<td class="text-right" style="vertical-align:middle;"></td>';
					$tablerow	.= ($dateDelayed > 0 && $dateDelayed < 31) ? '<td class="text-right" style="vertical-align:middle;">'.number_format($balance,2).'</td>' : '<td class="text-right" style="vertical-align:middle;"></td>';
					$tablerow	.= ($dateDelayed > 30 && $dateDelayed < 61) ? '<td class="text-right" style="vertical-align:middle;">'.number_format($balance,2).'</td>' : '<td class="text-right" style="vertical-align:middle;"></td>';
					$tablerow	.= ($dateDelayed > 60) ? '<td class="text-right" style="vertical-align:middle;">'.number_format($balance,2).'</td>' : '<td class="text-right" style="vertical-align:middle;"></td>';
					$tablerow	.= '<td class="text-right" style="vertical-align:middle;" >'.number_format($balance,2).'</td>';
					$tablerow	.= '</tr>';
			...
			$tablerow	.= '<td class="text-right" style="vertical-align:middle;" colspan="4"><strong>Total</strong>&nbsp;</td>';
			$tablerow	.= '<td class="text-right" style="vertical-align:middle;"><strong>'.number_format($totalcurrent,2).'</strong></td>';
			$tablerow	.= '<td class="text-right" style="vertical-align:middle;"><strong>'.number_format($total30,2).'</strong></td>';
			$tablerow	.= '<td class="text-right" style="vertical-align:middle;"><strong>'.number_format($total60,2).'</strong></td>';
			$tablerow	.= '<td class="text-right" style="vertical-align:middle;"><strong>'.number_format($totalover,2).'</strong></td>';
			$tablerow	.= '<td class="text-right" style="vertical-align:middle;"><strong>'.number_format($totalbalance,2).'</strong></td>';
			$tablerow	.= '</tr>';
		}else{
			$tablerow	.= '<tr>';
			$tablerow	.= '<td colspan = "9" class="text-center" >No Records Found</td>';
	...
	private function export()
	{
		...
		$retrieved = $this->ar_aging->fileExport($datefilter,$customer);
