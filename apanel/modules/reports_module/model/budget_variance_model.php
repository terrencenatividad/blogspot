...
class budget_variance_model extends wc_model {
	...
	public function getBudgetList($costcenter, $budget_type, $date) {
		...
		->leftJoin("(SELECT SUM(actual) as actual, SUM(allocated) as allocated, accountcode, budget_code FROM actual_budget WHERE id != '' AND voucherno NOT LIKE '%TMP_%' GROUP BY accountcode, budget_code)
