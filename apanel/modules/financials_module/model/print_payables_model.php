<?php
class print_payables_model extends fpdf {
	...
	public function Header() {
		...
		if ($documentinfo) {
			...
			$currencycode		= $documentinfo->currencycode;
		...
		$this->SetFont('Arial', 'B', 9);
		$this->Cell(30 ,$rowheight, 'Currency Code', 0, 0, 'L');
		$this->SetFont('Arial', '', 9);
		$this->Cell(40, $rowheight, $currencycode, 'B', 0, 'L');
	...
	private function drawDocumentDetails() {
		...
		$this->SetFont('Arial', 'B', '9');
		$this->Cell(100, 5, 'ACCOUNT CODE - ACCOUNT TITLE', 'LTB', 0, 'C', true);
		$this->Cell(30, 5, 'DEBIT', 'LTB', 0, 'C', true);
		$this->Cell(30, 5, 'CREDIT', 1, 0, 'C', true);
		$this->Cell(40, 5, 'CURRENCY AMOUNT', 1, 0, 'C', true);
		$this->Ln();
		
		if ($accounts) {
			$this->SetFont('Arial', '', '9');
			$this->setWidths(array(100, 30, 30, 40));
	...
	private function row($data, $type = 'accounts') {
		...
		$nb = 0;
		if ($type == 'accounts') {
			$col_index = array('accountname', 'debit', 'credit', 'currency');
