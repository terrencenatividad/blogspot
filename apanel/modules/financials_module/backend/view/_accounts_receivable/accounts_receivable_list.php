    ...
    <div class="box box-primary">
		<form method = "post">
			<div class="box-header">
				<div class="row">
					<div class = "col-md-8">
						<a class="btn btn-primary btn-flat" role="button" href="<?=BASE_URL?>financials/accounts_receivable/create" style="outline:none;">Create New Accounts Receivable</a>

						<!--<a class="btn btn-primary btn-flat" data-toggle = "modal" role="button" href = "#paymentModal" style="outline:none;">Receive Payments</a>-->
					...
					<table id="tableList" class="table table-hover">
						<!--<thead>
							<tr class = "info">
								...
								<th class = "col-md-1 text-center">Voucher No</th>
					...
					<form class="form-horizontal" id="paymentForm">
						<div class="row row-dense">
							<div class = "col-md-6 customer_div remove-margin">
								...
								<span class="help-block hidden small req-color" id = "customer_help"><i class="glyphicon glyphicon-exclamation-sign"></i> Please select a customer.</span> <!-- style = "margin-bottom: 0px" -->
							</div>
						...
						<div class="row">
							<div class="col-md-12 col-sm-12 col-xs-12 text-center">
								<div class="btn-group">
									<button type = "submit" class = "btn btn-primary btn-sm btn-flat" id="btnSave" onClick="applySelected(event);">Save&nbsp;</button>
								</div>
									&nbsp;&nbsp;&nbsp;
								<div class="btn-group">
...
function showList(pg) 
{
	...
	ajax.vendfilter      = $("#vendor").val();
	ajax.addCond 		 = $("#addCond").val();

	$.post('<?=BASE_URL?>financials/accounts_receivable/ajax/load_list', ajax, function(data)
	{
...
function showPayableList() 
{
	...
	$.post("<?= BASE_URL ?>financials/accounts_receivable/ajax/load_receivables",data)
	.done(function( data ) 
	{
...
$(document).ready(function() 
{
	...
	// 	function (start, end) {
	// 		$('#daterangefilter span').html(start.format('MMM DD, YYYY') + ' - ' + end.format('MMM DD, YYYY'));
	// 	}
	// );
	...
	// ajax.start = $("#daterangefilter").data('daterangepicker').startDate.format('YYYY-MM-DD');
	// ajax.end = $("#daterangefilter").data('daterangepicker').endDate.format('YYYY-MM-DD');
	...
	*/
	$('#deleteModal #btnYes').click(function() 
	{
		...
		// Update Details
		$.post("<?= BASE_URL ?>financials/accounts_receivable/ajax/update",{ table: "ar_details", condition: "voucherno = '"+id+"'", fields : column });

		// Update Header
		$.post("<?= BASE_URL ?>financials/accounts_receivable/ajax/update",{ table: "accountsreceivable", condition: "voucherno = '"+id+"'", fields : column })
		.done(function( data ) 
		{
			if(data.msg == "success")
			{
