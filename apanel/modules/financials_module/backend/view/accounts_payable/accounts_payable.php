									...
									<tbody>
										...
										<?
												...
												for($i = 0; $i < count($data["details"]); $i++)
												{
													...
													$debit_attr			= array();
													$credit_attr		= array();
													$taxcode 			= $data["details"][$i]->taxcode;
													$taxbase_amount 	= $data["details"][$i]->taxbase_amount;
						...
						<div class="row row-dense">
							<?php
								echo $ui->formField('text')
										...
										->setValue("")
										->draw($show_input);
								// <input class="form-control tin-input" maxlength="15" value="" name="tinno" id="tinno" placeholder="000-000-000-000" onkeyup="validateTIN('newVendor','tinno',this.value);" onblur="validateTIN('newVendor','tinno',this.value);" onkeypress="return isNumberKey(event,45);" type="text">
							?>
						</div>
						<div class="row row-dense">
							<?php
...
{		
	...
	{
		$.post('<?=BASE_URL?>financials/accounts_payable/ajax/save_data', $("#vendorDetailForm").serialize(), function(data) 
		{
			if(data.msg == "success")
			{
				field.readOnly 				= true;
...
function deleteItem(row)
{
	...
	if(rowindex.cells[0].childNodes[1] != null)
	{
		...
		if(rowCount > 3)
		{
			...
			{
				// console.log("1");
				$.post("<?=BASE_URL?>financials/accounts_payable/ajax/delete_row",{table:datatable,condition:condition})
				.done(function( data ) 
				{
					table.deleteRow(row);	
					resetIds();
...
function cancelTransaction(vno)
{
	...
	{	
		...
		$.post("<?=BASE_URL?>financials/accounts_payable/ajax/delete_row",data)
		.done(function(data1) 
		{
			if(data1.msg == "success")
			{
				$.post("<?=BASE_URL?>financials/accounts_payable/ajax/delete_row",data2)
				.done(function(data2) 
				{
	
					if(data2.msg == "success")
					{
						window.location.href = '<?=BASE_URL?>financials/accounts_payable';
					}
				});
			}
		});
...
function savePaymentRow(e,id)
{
	...
	if(valid == 0)
	{
		...
		var paymentvendor			= $('#receiptForm #vendor\\['+id+'\\]').val();
		...
		$.post("<?= BASE_URL ?>financials/accounts_payable/ajax/apply_payments",
		{
			"invoiceno[]": selected, 
...
$(document).ready(function() 
{
	...
	// Add new Vendor
	$("#newVendor #vendorBtnSave").click(function()
	{
		var valid	= 0;

		/**validate vendor fields**/
		valid		+= validateField('newVendor','partnercode', "partnercode_help");
		valid		+= validateField('newVendor','vendor_name', "vendor_name_help");
		valid		+= validateField('newVendor','address', "address_help");
		valid		+= validateField('newVendor','businesstype', "businesstype_help");

		if(valid == 0)
		{
			$.post('<?=BASE_URL?>financials/accounts_payable/ajax/save_data', $("#newVendor").serialize(), function(data) 
			{
				var optionvalue = $("#newVendor #partnercode").val();
				var optiondesc 	= $("#newVendor #vendor_name").val();

				if(data.msg == "success")
				{
					$('<option value="'+optionvalue+'">'+optiondesc+'</option>').insertAfter("#payableForm #vendor option:nth-child(4)");
					$('#payableForm #vendor').val(optionvalue);
					
					getPartnerInfo(optionvalue);

					$('#vendorModal').modal('hide');
					$('#vendorModal').find("input[type=text], textarea, select").val("");
					return true;
				}
				else
				{
	...
	$('#vendor_tin, #vendor_terms').on('keypress blur click', function(e) 
	{
		if(e.type == "keypress")
	...
	$('#vendor_address').on('blur click', function(e) 
	{
    	if(e.type == "blur")
			saveField(e.target.id);
		if(e.type == "click")
	...
	$('#rateForm #btnProceed').click(function(e)
	{
		...
		if(valid == 0)
		{
			if(parseFloat(amount) == 0)
			{
				if(accountentry == '')
				{
					$.post('<?=BASE_URL?>financials/accounts_payable/ajax/get_value', "account=" + account + "&event=exchange_rate")
					.done(function(data) 
					{
	...
	*/
	...
	$('#deletePaymentModal #btnYes').click(function() 
	{
		...
		$.post("<?= BASE_URL?>financials/accounts_payable/ajax/delete_payments", "voucher=" + id)
		.done(function( data ) 
		{	
			if(data.msg == "success")
			{
	...
	if('<?= $task ?>' == "create")
	{
		...
		$("#payableForm").change(function()
		{
			if($("#payableForm #accountcode\\[1\\]").val() != '' && $("#payableForm #document_date").val() != '' && (parseFloat($("#payableForm #debit\\[1\\]").val()) > 0 || parseFloat($("#payableForm #credit\\[1\\]").val()) > 0) && $("#payableForm #vendor").val() != '')
			{
				$.post("<?=BASE_URL?>financials/accounts_payable/ajax/save_payable_data",$("#payableForm").serialize())
				.done(function(data)
				{
					if(data.msg == "error")
					{
		...
		{
			$('#details_error').removeClass('has-error');
			var valid	= 0;
			...
			if(valid == 0)
			{
				...
				$('#details_error').addClass('has-error');
				var next = $('#payableForm').find(".has-error").first();
	...
	if(hash != '')
	{
		...
		if(noCashAccounts == true)
		{
			...
			{
				location.href	= "<?= BASE_URL ?>financials/accounts_payable";
			});
		}
	}
	else
	...
	$('#proformacode').change(function()
	{
		...
		$.post("<?= BASE_URL ?>financials/accounts_payable/ajax/apply_proforma",{code:code})
		.done(function(data) 
		{
			...
			var table 		= document.getElementById('itemsTable');
			var count		= table.rows.length - 3;
			var firstaccount= $('#payableForm #accountcode\\[1\\]').val();
...
$('#tax_apply').click(function(){
	...
	$.post("<?= BASE_URL ?>financials/accounts_payable/ajax/get_account",{tax_account:tax_account,tax_amount:tax_amount})
	.done(function(data) {
...
$('#itemsTable').on('change', '.accountcode', function(){
	row = $(this).closest('tr')
	var account = $(this).val();
	get_coa(account);
})

$('#itemsTable .taxcode').each(function(){
	var acc = $(this).val();
