...
class controller extends wc_controller 
{
	public function __construct()
	{
		parent::__construct();
		$this->url 			    = new url();
		$this->po 				= new import_purchaseorder();
		...
		$this->view->header_active = 'purchase/import_purchaseorder/';
	...
	public function listing()
	{
		...
		$this->view->load('import_purchaseorder/import_purchaseorderlist', $data);
	}

	public function create($request_no="")
	{
		$this->view->title      = 'Create Import Purchase Order';
		...
		$gen_value               = $this->po->getValue("import_purchaseorder", "COUNT(*) as count", "voucherno != ''");
		...
		if( $save_status == "final" || $save_status == "final_preview" || $save_status == "final_new" )
		{
			...
			$isExist 				= $this->po->getValue("import_purchaseorder", array("voucherno"), "voucherno = '$voucherno'");
			// var_dump($isExist);
			if($isExist[0]->voucherno)
			{
				...
				$generatedvoucher			= $this->seq->getValue('IPO', $this->companycode); 
				...
				$updateTempRecord			= $this->po->updateData($update_info,"import_purchaseorder",$update_condition);
				$updateTempRecord			= $this->po->updateData($update_info,"import_purchaseorder_details",$update_condition);
				...
				{
					$this->url->redirect(BASE_URL . 'purchase/import_purchaseorder');
				}
				else if( $updateTempRecord && $save_status == 'final_preview' )
				{
					$this->url->redirect(BASE_URL . 'purchase/import_purchaseorder/view/'.$generatedvoucher);
				}
				else if( $updateTempRecord && $save_status == 'final_new' )
				{
					$this->url->redirect(BASE_URL . 'purchase/import_purchaseorder/create');
				}
			}
	...
	public function tag_as_complete($voucherno)
	{
		...
		$result 	=	$this->po->updateData($data, "import_purchaseorder", " voucherno = '$voucherno' AND stat != 'posted' ");

		if( $result )
		{
			$result 	=	$this->po->updateData($data, "import_purchaseorder_details", " voucherno = '$voucherno' AND stat != 'posted' ");
	...
	public function print_preview($voucherno) 
	{
		...
		$vendorcode 		=	$this->po->getValue("import_purchaseorder", array('vendor')," voucherno = '$voucherno'");
		...
		foreach ($documentcontent as $key => $row) {
			...
			$row->onhandqty		= number_format($row->onhandqty);
			$row->quantity		= number_format($row->quantity);
			$row->price			= number_format($row->price, 2);
			$row->amount		= number_format($row->amount, 2);
			$row->discount		= number_format($row->discount, 2);
			$row->convertedamount		= number_format($row->convertedamount, 2);
	...
	public function ajax($task)
	{
		...
		else if( $task == 'get_exchange_rate' )
		{
			$result = $this->get_exchange_rate();
		}
	...
	private function get_exchange_rate()
	{
		...
		$fields 	= 	array('basecurrencycode','exchangecurrencycode','exchangerate');

		$result 	=	$this->po->getValue('exchangerate', $fields, " exchangecurrencycode = '$currency' ", "");
		foreach($result as $row)
		{
			$exchangerate = $row->exchangerate;
		}
		
		return $dataArray = array("exchangerate" => $exchangerate);
	}	
		...
		else
		{
			...
			if(isset($data_post['budgetcode'])){
				for($i=1;$i<=count($data_post['budgetcode']);$i++){
					...
					$saveArr['accountcode'] = $result['accountcode'];
					$saveArr['budget_code'] = $data_post['budgetcode'][$i];
					$saveArr['actual'] 		= str_replace(',','', $data_post['baseamount'][$i]);
					$actualArr[]      		= $saveArr;
				}
			}
	...
	private function cancel()
	{
		...
		$result 	=	$this->po->delete_temp_transactions($voucher, "import_purchaseorder", "import_purchaseorder_details");
	...
	private function delete_po()
	{
		...
		$result 	=	$this->po->updateData($data, "import_purchaseorder", $cond);

		if( $result )
		{
			$result 	=	$this->po->updateData($data, "import_purchaseorder_details", $cond);
		}
