	...
	class supplier extends wc_model
	{
		public function retrieveListing($search, $sort)
		{
			...
			return $this->db->setTable('partners p')
							->setFields($fields)
							->setWhere(" p.partnertype = 'supplier' $add_cond ")
							->setOrderBy($sort)
							->runPagination();
		}
		...
		public function export($search, $sort)
		{
			...
			$fields 	=	array("p.partnercode","p.partnername","p.first_name","p.last_name","p.address1","p.email","p.businesstype","p.tinno","p.terms","p.mobile","p.stat");
