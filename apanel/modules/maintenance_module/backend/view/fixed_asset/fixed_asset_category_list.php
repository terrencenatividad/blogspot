    ...
    <div class="box box-primary">
        ...
        <div class="box-header">
            <div class="row">
				...
				<div class = "col-md-8">
				    <a href="<?php MODULE_URL ?>add_category" class="btn btn-primary btn-flat" role="button">Add Category</a>
                    <a href="<?php MODULE_URL ?>listing" class="btn btn-primary btn-flat" role="button">Back to Fixed Asset</a>
                ...
                <thead>
					<?php
						echo $ui->loadElement('table')
								...
								->addHeader('Description', array('class'=>'col-md-6'),'sort','description')							
								->draw();
					?>
				</thead>
...
function showList(){
	$.post('<?=BASE_URL?>maintenance/fixed_asset/ajax/ajax_list_category', ajax, function(data)
	{
...
$(document).ready(function() 
{
	...
	$( "#currency_table" ).on('click' , '.delete', function() 
	{
		...
		if( id != "" )
		{
			...
			{
				$.post('<?=BASE_URL?>maintenance/fixed_asset/ajax/delete_category', 'id=' + id, function(data) 
				{
					if( data.msg == 'success' )	
					{
						$(".delete-modal").modal("hide");
					...
					{			
						$(".delete-modal").modal("hide");
						show_error("Unable to delete the Fixed Asset.");
					}
				});
			});	
		}
...
$('#items').on('change', function(){
	...
	$('#currency_table').on('click', '.edit', function() {
    var id = $(this).attr('data-id');
        window.location =  "<?=MODULE_URL?>edit_category/"+id;
    });
	$('#currency_table').on('click', '.view', function() {
    var id = $(this).attr('data-id');
        window.location =  "<?=MODULE_URL?>view_category/"+id;
    });
