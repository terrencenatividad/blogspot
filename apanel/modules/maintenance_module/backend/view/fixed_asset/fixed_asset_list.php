    ...
    <div class="box box-primary">
        ...
        <div class="box-header">
            <div class="row">
				<div class = "col-md-8">
					<?=$ui->CreateNewButton('');?>
                    <a href="<?php MODULE_URL ?>listing_category" class="btn btn-primary btn-flat" role="button">Fixed Asset Category</a>
                ...
                <thead>
					<?php
						echo $ui->loadElement('table')
								...
								->addHeader('Item No',array('class'=>'col-md-3'),'sort','itemno')
								->addHeader('Description', array('class'=>'col-md-3'),'sort','description')
								->addHeader('Category', array('class'=>'col-md-3'),'sort','category')								
								->addHeader('Location', array('class'=>'col-md-3'),'sort','location')								
								->addHeader('Amount', array('class'=>'col-md-3'),'sort','amount')								
								->draw();
					?>
				</thead>
...
function showList(){
	$.post('<?=BASE_URL?>maintenance/fixed_asset/ajax/ajax_list', ajax, function(data)
	{
...
$(document).ready(function() 
{
	...
	$( "#currency_table" ).on('click' , '.delete', function() 
	{
		...
		if( id != "" )
		{
			...
			{
				$.post('<?=BASE_URL?>maintenance/fixed_asset/ajax/delete', 'id=' + id, function(data) 
				{
					if( data.msg == 'success' )	
					{
						$(".delete-modal").modal("hide");
